...
def _write_pages(pdf: Path, select_indices: Callable[[int], Iterable[int]]) -> bytes:
    buffer = BytesIO()
...
def extract_page(pdf: Path, index: int) -> bytes:
    return _write_pages(pdf, lambda page_count: [_normalize_index(index, page_count)])


def delete_multiple_pages(pdf: Path, start: int = 0, stop: int | None = None) -> bytes:
...
def delete_page(pdf: Path, index: int) -> bytes:
    def select_indices(page_count: int) -> Iterable[int]:
